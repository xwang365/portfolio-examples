...
def main():
    ...
    kriging_mat = numpy.float16(kriging_mat)
    b = numpy.float16(b)
    print('kriging_mat.shape: {}'.format(kriging_mat.shape))
    print('b.shape: {}'.format(b.shape))
    ...
    with tf.device("cpu"):
        p_A = tf.compat.v1.placeholder(numpy.float16, kriging_mat.shape,  name="A")
        p_b = tf.compat.v1.placeholder(numpy.float16, b.shape, name="b")
    ...
    with tf.compat.v1.Session() as sess:
        ...
        cmp_diff = numpy.abs(mat_solve_res[0] - x)
        max_diff = numpy.max(cmp_diff)
        min_diff = numpy.min(cmp_diff)
        avg_diff = numpy.average(cmp_diff)
        print('max_diff: {:.8f}, min_diff: {:.8f}, avg_diff: {:.8f}'.format(max_diff, min_diff, avg_diff))
