...
def read_tf_records(batch_size, tf_records, num_repeats=1,
                    shuffle_records=True, shuffle_examples=False,
    ...
    Args:
        ...
        interleave: whether to interleave examples from multiple tf_records
    ...
    if interleave:
        ...
        dataset = record_list.apply(tf.contrib.data.parallel_interleave(
            map_func, cycle_length=n_files, sloppy=True))
    else:
    ...
    # Deactivated because they actually reduce performance
    options.experimental_optimization.autotune_buffers = False
    options.experimental_optimization.parallel_batch = False
    # Currently not available in Graphcore's TensorFlow.
...
def get_input_tensors(batch_size, tf_records, num_repeats=1,
    ...
    dataset = dataset.prefetch(tf.contrib.data.AUTOTUNE).cache()
    return dataset  # .make_one_shot_iterator().get_next()


def get_ipu_input_tensors(batch_size, tf_records, num_repeats=1,
                          shuffle_records=False, shuffle_examples=False,
                          shuffle_buffer_size=None,
                          filter_amount=0.05, random_rotation=True):
    """Optimized data version for better data throughput for the IPU."""
    print("Reading tf_records from {} inputs".format(len(tf_records)))

    # Data has already been shuffled when storing. (see ExampleBuffer.flush)
    shuffle_examples = False
    ...
    dataset = dataset.map(
        functools.partial(batch_parse_tf_example, batch_size), num_parallel_calls=40)

    if random_rotation:
        dataset = dataset.map(_random_rotation_pyfunc, num_parallel_calls=40)
    dataset = dataset.prefetch(tf.contrib.data.AUTOTUNE).cache().repeat()
