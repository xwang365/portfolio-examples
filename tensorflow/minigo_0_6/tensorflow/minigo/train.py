...
from tensorflow.python.ipu.dataset_benchmark import dataset_benchmark, infeed_benchmark
...
DATA_BENCHMARK = False
INFEED_BENCHMARK = False
DATA_PROFILE = False
...
def train(*tf_records: "Records to train on"):
    ...
    if INFEED_BENCHMARK:
        benchmark_op = infeed_benchmark(
            infeed_queue=ipu_infeed_queue.IPUInfeedQueue(_input_fn(), feed_name="infeed"),
            number_of_epochs=80,
            elements_per_epochs=10000,
            print_stats=True,
            # apply_options=False
...
def serving_input_receiver_fn():
    """ Helper function for storing SavedModel
    """
    feature = tf.placeholder(
        tf.float32, [None, go.N, go.N, features_lib.NEW_FEATURES_PLANES],
        name='pos_tensor')
    label = tf.placeholder(tf.float32, [None, go.N * go.N + 2], name="labels")
    inputs = {
        "features": feature,
        "labels": label
    }
    ...
    return tf.estimator.export.build_raw_serving_input_receiver_fn(
        inputs, default_batch_size=None
    )
...
def main(argv):
    ...
    if FLAGS.export_path:
        ...
        estimator.export_saved_model(FLAGS.export_path, serving_input_receiver_fn())
    else:
