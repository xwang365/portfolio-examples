...
os.environ['TF_CPP_VMODULE'] = 'poplar_compiler=1,poplar_executable=1'
...
flags.DEFINE_bool('use_ipu', True, 'Whether to use IPU for training.')
...
flags.DEFINE_integer(
    'num_ipu_cores', default=16,
    help=('Number of IPU cores. For a single cauldron, this is 16 because each'
          ' cauldron has 8 cards with 2 chips.'))
...
flags.DEFINE_string("PROFILING", "", "Which component to profile.")
...
class DualNetwork():
    def __init__(self, save_file):
        ...
        if not params['use_ipu']:
            config.gpu_options.allow_growth = True

        self.sess = tf.Session(graph=tf.Graph(), config=config)
        self.initialize_graph()
        # Test code to verify interfaces and compilation
        batch_size = 48
        a = self.run_many([go.Position()]*batch_size)
        b = self.run_batch_test(go.Position(), batch_size)

    def initialize_graph(self):
        params = FLAGS.flag_values_dict()

        def model_predictions(features):
            return model_inference_fn(
                features, False,
                params=params)
        ...
        with self.sess.graph.as_default():
            ...
            logging.debug('TPU inference is supported on C++ only. '
    ...
    def initialize_weights(self, save_file):
        ...
        without redefining the entire graph."""
    ...
    def run_batch_test(self, position, batch_size, use_random_symmetry=True):
        ...
        outputs = self.sess.run(
            self.inference_output, feed_dict={
                self.inference_input: [x_data]*batch_size})
        probabilities, value = outputs['policy_output'], outputs['value_output']
...
def _get_ipu_estimator(model=model_fn, num_replicas=None, num_shards=1, iterations_per_loop=None):
    if num_replicas is None:
        num_replicas = FLAGS.num_ipu_cores
        print("Using %i replicas for training." % num_replicas)

    if iterations_per_loop is None:
        iterations_per_loop = FLAGS.iterations_per_loop
    if iterations_per_loop is None:
        iterations_per_loop = 100

    working_dir = FLAGS.work_dir
    hparams = FLAGS.flag_values_dict()

    tf_estimator = model  # _get_nontpu_estimator()  #

    profiling = FLAGS.PROFILING

    if profiling in ["train", "eval", "puzzle"]:
    ...
    if profiling in ["train", "eval", "puzzle"]:
        ipu_run_config = ipu.ipu_run_config.IPURunConfig(
            # profiling needs short cycles
            iterations_per_loop=4,
            ipu_options=ipu_options,
            num_replicas=num_replicas,
            num_shards=num_shards,
            autosharding=num_shards > 1
        )
    else:
        ipu_run_config = ipu.ipu_run_config.IPURunConfig(
    ...
    estimator = IPUEstimator(
        tf_estimator, model_dir=working_dir, params=hparams,
        config=config)
    return estimator
...
def freeze_graph(model_path):
    try:
        ...
        print("IPU already configured!")
    n = DualNetwork(model_path)
    with n.sess.graph.as_default():
        tf.train.Saver().save(n.sess, model_path + "_raw.ckpt")
    if model_path.endswith("target") or model_path.endswith("model.ckpt-9383"):
