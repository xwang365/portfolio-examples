...
class BertPretrainingAccuracy(paddle.nn.Layer):
    def __init__(self,
                 ...
                 batch_size,
                 ignore_index=-1):
        super(BertPretrainingAccuracy, self).__init__()
        self.mlm_label = mlm_label
        self.nsp_label = nsp_label
    ...
    def forward(self, prediction_scores, seq_relationship_score):
        ...
        with paddle.fluid.ipu_shard(ipu_index=0, ipu_stage=self.num_ipus):
            with paddle.static.name_scope("Acc"):
                mlm_pred = paddle.fluid.layers.argmax(
                    prediction_scores, axis=-1)
                nsp_pred = paddle.fluid.layers.argmax(
                    seq_relationship_score, axis=-1)
                mlm_pred = paddle.cast(mlm_pred, "int32")
                nsp_pred = paddle.cast(nsp_pred, "int32")
                mlm_label = paddle.cast(self.mlm_label, "int32")
                nsp_label = paddle.cast(self.nsp_label, "int32")
                ...
                if self.enable_fp16:
                    mlm_mask = paddle.cast(mlm_mask, "float16")
                    mlm_correct = paddle.cast(mlm_correct, "float16")
                else:
                ...
                mlm_pred_acc = paddle.fluid.layers.elementwise_div(
                    total_correct_tokens, total_tokens)

                nsp_correct = paddle.fluid.layers.equal(nsp_pred, nsp_label)
                if self.enable_fp16:
                    nsp_correct = paddle.cast(nsp_correct, "float16")
                else:
                    nsp_correct = paddle.cast(nsp_correct, "float32")
                nsp_pred_acc = paddle.mean(nsp_correct)

                return mlm_pred_acc, nsp_pred_acc
...
class BertPretrainingCriterion(paddle.nn.Layer):
    ...
    def __init__(self, vocab_size, num_ipus, ignore_index=-1):
    ...
    def forward(self, prediction_scores, seq_relationship_score,
        ...
        """
        #with paddle.static.amp.fp16_guard():
        logging.info("Los Layer - ipu_index:%d, ipu_stage:%d" %
                     (0, self.num_ipus))
        with paddle.fluid.ipu_shard(ipu_index=0, ipu_stage=self.num_ipus):
            with paddle.static.name_scope("Loss"):
                masked_lm_softmax = paddle.fluid.layers.softmax(
                    prediction_scores)
                attrs = {
                    'name': 'loss_const',
                    'shape': [1],
                ...
                loss_const = paddle.fluid.layers.fill_constant(**attrs)

                masked_lm_softmax = paddle.fluid.layers.elementwise_add(
                    masked_lm_softmax, loss_const)
                masked_lm_loss = paddle.fluid.layers.cross_entropy(
                    masked_lm_softmax,
                    masked_lm_labels,
                    ignore_index=self.ignore_index)

                masked_lm_scale = paddle.fluid.layers.reduce_sum(
                    masked_lm_scale)
                masked_lm_loss = masked_lm_loss / masked_lm_scale
                masked_lm_loss = paddle.sum(masked_lm_loss)

                next_sentence_softmax = paddle.fluid.layers.softmax(
                    seq_relationship_score)
                next_sentence_softmax = paddle.fluid.layers.elementwise_add(
                    next_sentence_softmax, loss_const)
                next_sentence_loss = paddle.fluid.layers.cross_entropy(
                    next_sentence_softmax, next_sentence_labels)
                next_sentence_loss = paddle.mean(next_sentence_loss)

                return masked_lm_loss + next_sentence_loss, masked_lm_loss, next_sentence_loss
