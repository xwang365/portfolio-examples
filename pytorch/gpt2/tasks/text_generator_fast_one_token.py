...
from transformers import (BertTokenizer, GPT2Config, GPT2Model)
...
def set_args():
    ...
    parser.add_argument("--model-path", type=str, default="uer/gpt2-chinese-poem", help="path of model to load")
    ...
    parser.add_argument("--fp16", type=str_to_bool, nargs="?", const=True, default=False,
                        help="fp16")
    parser.add_argument("--start_id", type=int, default=101, help='start token id')
    parser.add_argument("--end_id", type=int, default=102, help='end token id')
...
class GTP2Wrapper(torch.nn.Module):
    ...
    def forward(self, context, dynamic_mask, position_ids):
        if self.args.poptorch_loop:
            ### 1 stage #####
            kv_size = (self.model.config.batch, self.model.config.n_head, self.model.config.seq, int(self.model.config.n_embd/self.model.config.n_head))
            #### past key value ###
            ...
            def body(context, dynamic_mask, position_ids, record, past_keys, past_values):
                ...
                next_token_logits = torch.matmul(hidden_states[0], self.model.wte.weight.T).view(self.args.batch_size, -1)
                (next_token_value, next_token) = torch.topk(next_token_logits, 1)
                #####
                next_dynamic_mask = torch.cat((torch.ones(self.args.batch_size, 1).to(torch.int64), dynamic_mask[:, :-1]), dim=-1)
                next_id = next_token
            ...
            new_context, dynamic_mask, new_position_ids, new_record, new_past_keys_tensor, new_past_values_tensor = poptorch.for_loop(self.count-1, body, [new_context, dynamic_mask, new_position_ids, new_record, new_past_keys_tensor, new_past_values_tensor])
            return new_record
        else:
            hidden_states = self.model(context, attention_mask=dynamic_mask,
...
def main():
    ...
    if args.model_path:
        ...
        tokenizer = Tokenizer.from_file("./data/gpt2-bpe-tokenizer.json")
    ...
    while True:
        try:
            # text =  "梅 山 如 积 翠 ，"
            text = input("Input: ")
            text_ids = [args.start_id] + tokenizer.encode(text, add_special_tokens=False)
            ...
            if args.poptorch_loop:
                ...
                for _ in range(args.input_len + args.output_len):
                    ...
                    for batch in range(args.batch_size):
                        if args.end_id in all_ids[batch, :]:
                            finished_sample += 1
