...
def get_args():
    ...
    parser.add_argument("--strict-lambada", type=str_to_bool, nargs="?", const=True, default=False, help="strict-lambada")
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(data_loader):
            ...
            if iteration % args.log_interval == 0:
                print('> working on iteration: {}, correct: {}/{}'.format(iteration, int(output.sum()), args.batches_per_step))

            total_output += output.float().sum()

        num_examples = len(data_loader.dataset)
        acc = total_output / num_examples
        print("number correct: {:.4E}, total examples: {:.4E}, avg accuracy: {:.4E}".format(total_output, num_examples, acc))
