...
from utils import _get_layer_ipu, _WorkerInit, str_to_bool
...
def get_args():
    ...
    parser.add_argument('--tokenizer-type', type=int, default=0, help='0: transformers.tokenizer, 1: Megatron.tokenizer')
    parser.add_argument('--overlapping-eval', type=int, default=32, help='Sliding window for overlapping evaluation.')
    parser.add_argument('--pretrained-checkpoint', default='', type=str, required=True, help='pretrained checkpoint to load')
...
class GTP2Wrapper(nn.Module):
    ...
    def forward(self, input_ids, labels, loss_mask):
        ...
        losses = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))
...
def main():
    ...
    # Required for gpt-large and gpt-xl, since onnx modelProto > 2GB
    ...
    if args.tokenizer_type == 0:
        ...
        tokenizer = build_megatron_tokenizer(vocab_file="./tokenizer/gpt2-vocab-50256.json", merge_file="./tokenizer/gpt2-merges-50256.txt")
    ...
    with torch.no_grad():
        ...
        for iteration, batch in enumerate(data_loader):
            ...
            if iteration % args.log_interval == 0:
                print('> working on iteration: {}, loss: {}'.format(iteration, output.mean()))
        ...
        print("avg loss: {:.4E}, ppl: {:.4E}, adjusted ppl: {:.4E}, token ratio: {}".format(val_loss, ppl, adjusted_ppl, token_ratio))
